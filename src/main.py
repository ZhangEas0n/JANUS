...
from src.data.modules.speech.fluent_speech_commands import (
    FluentSpeechCommandsDataModuleConfig,
    FluentSpeechCommandsDataModule,
)
...
def construct_data_module(
    ...
    elif isinstance(dm_cfg, FluentSpeechCommandsDataModuleConfig):
        if not isinstance(dl_cfg, SpeechDataLoaderConfig):
            raise ValueError(
                f"FluentSpeechCommandsDataModule expects {SpeechDataLoaderConfig},"
                f" got {dl_cfg}"
            )

        dm = FluentSpeechCommandsDataModule(
            cfg=dm_cfg,
            dl_cfg=dl_cfg,
            tokenizer=construct_tokenizer(cfg),
            train_pipeline=train_pipeline,
