    ...
    def __init__(self, huggingface_wav2vec2_ctc_id: Optional[str]):
        ...
        if huggingface_wav2vec2_ctc_id is not None:
            if AutoModelForCTC is not None:
                tmp_init = AutoModelForCTC.from_pretrained(huggingface_wav2vec2_ctc_id)
            elif "hubert" in huggingface_wav2vec2_ctc_id.lower():
                tmp_init = HubertForCTC.from_pretrained(huggingface_wav2vec2_ctc_id)
            else:
