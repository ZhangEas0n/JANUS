...
def _cfg_get(cfg: DictConfig, key: str, default):
    if "hubert_eval" in cfg and key in cfg.hubert_eval:
        return cfg.hubert_eval[key]
    return default
...
def evaluate_hubert_sv(
    ...
    sv_cfg.network.wav2vec_hunggingface_id = str(hubert_backbone)
    sv_cfg.load_network_from_checkpoint = sv_ckpt

    print(
        "[*] Initializing HuBERT SV evaluation: "
        f"backbone={hubert_backbone}, sv_ckpt={sv_ckpt}"
    )
...
def evaluate_hubert_sr(
    ...
    print(
        "[*] Initializing HuBERT SR evaluation: "
        f"backbone={hubert_backbone}, intent_head={intent_head_path}"
    )
    ...
    sr_cfg.network.wav2vec_hunggingface_id = str(hubert_backbone)

    # HuBERT-base-ls960 is used here as an encoder. Keep the existing local
    # wav2vec2 tokenizer for FSC CTC data-module construction.
...
def main_evaluation(cfg: DictConfig):
    ...
    hubert_backbone = Path(
        _cfg_get(cfg, "hubert_backbone", project_root / "model" / "hubert-base-ls960")
    )
    if not hubert_backbone.exists():
        raise FileNotFoundError(f"Missing local HuBERT files: {hubert_backbone}")
    ...
    intent_head_path = Path(
        _cfg_get(
            cfg,
            "intent_head_path",
            project_root / "intent" / "hubert-base-intent.pt",
        )
    )
    sv_ckpt = _optional_path(
        _cfg_get(cfg, "sv_ckpt", "/home/zyc/advpre_sv/model/hubert_sv.ckpt")
    )
    ...
    output_path = Path(
        _cfg_get(
            cfg,
            "output_path",
            project_root / "results" / "main" / "hubert_btuap_eval.json",
        )
    )

    print(
        "[*] HuBERT BTUAP evaluation config: "
        f"hubert_backbone={hubert_backbone}, uap_path={uap_path}, "
    ...
    sr_results = None
    if run_sv:
        sv_results = evaluate_hubert_sv(
            cfg=cfg,
            evaluator=evaluator,
            hubert_backbone=hubert_backbone,
            sv_ckpt=sv_ckpt,
            uap_delta=uap_delta,
            device=device,
    ...
    with output_path.open("w", encoding="utf-8") as handle:
        json.dump(
            {
                "config": {
                    "hubert_backbone": str(hubert_backbone),
    ...
    print(f"[*] Saved HuBERT BTUAP evaluation: {output_path}")
