...
def _cfg_get(cfg: DictConfig, key: str, default):
    if "cw" in cfg and key in cfg.cw:
        return cfg.cw[key]
    return default
...
def _project_delta_to_min_snr(x_clean_raw, delta, valid_mask, min_snr_db):
    if min_snr_db is None:
        return delta

    reduce_dims = tuple(range(1, x_clean_raw.dim()))
    valid_count = valid_mask.sum(dim=reduce_dims, keepdim=True).clamp_min(1.0)
    signal_power = (
        (x_clean_raw * valid_mask).pow(2).sum(dim=reduce_dims, keepdim=True)
        / valid_count
    ).clamp_min(1e-12)
    noise_power = (
        (delta * valid_mask).pow(2).sum(dim=reduce_dims, keepdim=True)
        / valid_count
    ).clamp_min(1e-20)
    max_noise_power = signal_power / (10.0 ** (float(min_snr_db) / 10.0))
    scale = torch.sqrt(max_noise_power / noise_power).clamp(max=1.0)
    return delta * scale


def _cw_sensitive_attack(
    sr_model,
    intent_head,
    x_clean_raw,
    input_lengths,
    ...
    delta = torch.zeros_like(x_clean_raw, requires_grad=True)
    ...
    for _ in range(steps):
        ...
        p_sens = probs[:, sensitive_class_ids].sum(dim=-1)
        other_prob = 1.0 - p_sens
        if optimize_mask is not None and optimize_mask.any():
            p_sens = p_sens[optimize_mask]
            other_prob = other_prob[optimize_mask]

        attack_loss = torch.clamp(other_prob - p_sens + confidence, min=0.0).mean()
        l2 = ((delta * valid_mask).pow(2).sum() / valid_mask.sum().clamp_min(1.0))
        loss = attack_loss + c_l2 * l2
        ...
        with torch.no_grad():
            ...
            delta.mul_(valid_mask)
            delta.copy_(torch.clamp(x_clean_raw + delta, -1.0, 1.0) - x_clean_raw)
            delta.mul_(valid_mask)
            delta.copy_(
                _project_delta_to_min_snr(
                    x_clean_raw,
                    delta,
                    valid_mask,
                    min_snr_db,
...
def _cw_sensitive_attack_chunked(
    sr_model,
    intent_head,
    x_clean_raw,
    input_lengths,
    ...
    for sample_idx, valid_length in enumerate(model_lengths):
        ...
        chunk_pbar = tqdm(
            total=num_chunks,
            desc=f"CW chunks sample {sample_idx + 1}/{len(model_lengths)}",
            leave=False,
        ...
        while start < valid_length:
            ...
            adv_chunk = _cw_sensitive_attack(
                sr_model=sr_model,
                intent_head=intent_head,
                x_clean_raw=chunk,
...
def _collect_sv_embeddings(
    ...
    pbar = tqdm(total=total_batches, desc="SV CW eval", leave=True)
    for batch_idx, batch in enumerate(dataloader):
        ...
        if use_cw:
            ...
            adv_raw = _cw_sensitive_attack_chunked(
                sr_model=sr_model,
                intent_head=intent_head,
                x_clean_raw=x_clean_raw,
                input_lengths=input_lengths,
        ...
        pbar.update(1)
        pbar.set_postfix(mode="attack" if use_cw else "clean")
...
def evaluate_sr_cw(
    ...
    total_batches = _safe_progress_total(dataloader, max_batches)
    pbar = tqdm(total=total_batches, desc="SR CW eval", leave=True)

    for batch in dataloader:
        if max_batches is not None and batches_seen >= max_batches:
            break
        samples_collected = sum(probabilities.numel() for probabilities in clean_probabilities)
        ...
        evaluation_mask = benign_mask.clone()
        if max_samples is not None:
            remaining = max_samples - samples_collected
            benign_indices = torch.nonzero(benign_mask, as_tuple=False).flatten()
            evaluation_mask.zero_()
            evaluation_mask[benign_indices[:remaining]] = True
        ...
        adv_raw = _cw_sensitive_attack_chunked(
            ...
            max_attack_samples=max_attack_samples,
            min_snr_db=min_snr_db,
            optimize_mask=evaluation_mask,
            show_chunk_progress=True,
        )
        selected_lengths = [
            length
            for length, selected in zip(model_lengths, evaluation_mask.tolist())
            if selected
        ]
        linf, rms, snr = _perturbation_stats(
            raw_audio[evaluation_mask],
            adv_raw[evaluation_mask],
            selected_lengths,
        )
        linf_sum += linf
        rms_sum += rms
        ...
        clean_probabilities.append(clean_p_sens[evaluation_mask].cpu())
        adv_probabilities.append(adv_p_sens[evaluation_mask].cpu())
        del batch, raw_audio, adv_raw, adv_input, adv_embedding, adv_logits
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
        pbar.update(1)
        selected_count = int(evaluation_mask.sum().item())
        pbar.set_postfix(
            benign=selected_count,
            evaluated=samples_collected + selected_count,
            attacked=stat_batches,
            p_adv=f"{adv_p_sens[evaluation_mask].mean().item():.3f}",
        )

    pbar.close()

    if not clean_probabilities:
        raise ValueError("No benign samples available for SR CW evaluation.")
...
def main_evaluation(cfg: DictConfig):
    ...
    steps = int(_cfg_get(cfg, "steps", 50))
    c_l2 = float(_cfg_get(cfg, "c_l2", 0.01))
    confidence = float(_cfg_get(cfg, "confidence", 0.0))
    min_snr_db = _cfg_get(cfg, "min_snr_db", None)
    min_snr_db = None if min_snr_db is None else float(min_snr_db)
    if min_snr_db is not None and min_snr_db <= 0:
        raise ValueError("cw.min_snr_db must be positive or null.")
    ...
    if max_attack_samples is not None and max_attack_samples <= 0:
        raise ValueError("cw.max_attack_samples must be positive or null.")
    if max_attack_samples is not None and max_attack_samples < 400:
        raise ValueError("cw.max_attack_samples must be at least 400 for wav2vec2.")
    ...
    sr_max_samples = _cfg_get(cfg, "sr_max_samples", None)
    ...
    sr_max_samples = None if sr_max_samples is None else int(sr_max_samples)
    if sr_max_samples is not None and sr_max_samples <= 0:
        raise ValueError("cw.sr_max_samples must be positive or null.")
    print(
        "[*] C&W-style baseline: "
        f"epsilon={epsilon}, learning_rate={learning_rate}, steps={steps}, "
        f"c_l2={c_l2}, confidence={confidence}, "
        f"min_snr_db={min_snr_db}, "
        ...
        f"sr_max_samples={sr_max_samples}"
    )
    ...
    if run_sv:
        ...
        print("[*] Running SV C&W transfer evaluation...")
        evaluate_sv_cw(
            victim_model=victim_model,
            sr_model=sr_model,
            intent_head=intent_head,
        ...
        if run_sr:
            print("[*] Releasing SV victim model before SR C&W evaluation...")
            victim_model.to("cpu")
            del victim_model
            if torch.cuda.is_available():
                torch.cuda.empty_cache()
    ...
    if run_sr:
        print("[*] Running SR C&W evaluation on FSC test benign samples...")
        evaluate_sr_cw(
            sr_model=sr_model,
            intent_head=intent_head,
