...
def _cfg_get(cfg: DictConfig, key: str, default):
    if "large_eval" in cfg and key in cfg.large_eval:
        return cfg.large_eval[key]
    return default
...
def evaluate_large_sv(
    ...
    sv_cfg.network.wav2vec_hunggingface_id = str(large_backbone)
    sv_cfg.load_network_from_checkpoint = sv_ckpt

    print(
        "[*] Initializing large SV evaluation: "
        f"backbone={large_backbone}, sv_ckpt={sv_ckpt}"
    )
...
def evaluate_large_sr(
    ...
    print(
        "[*] Initializing large SR evaluation: "
        f"backbone={large_backbone}, intent_head={intent_head_path}"
    )
    ...
    sr_cfg.network.wav2vec_hunggingface_id = str(large_backbone)
    sr_cfg.tokenizer.tokenizer_huggingface_id = str(large_backbone)
...
def compute_risk_overall(sv_results, sr_results):
    ...
    conditional_flip = (
        None if sr_results is None else float(sr_results["conditional_flip_rate"])
    )
    if asr_sv is None or conditional_flip is None:
        risk_overall = None
    else:
        risk_overall = 0.5 * asr_sv + 0.5 * conditional_flip
    return {
        "asr_sv": asr_sv,
        "conditional_flip_rate": conditional_flip,
...
def main_evaluation(cfg: DictConfig):
    ...
    large_backbone = Path(
        _cfg_get(cfg, "large_backbone", project_root / "model" / "wav2vec2-large-960h")
    )
    if not large_backbone.exists():
        raise FileNotFoundError(f"Missing local large wav2vec2 files: {large_backbone}")
    ...
    intent_head_path = Path(
        _cfg_get(cfg, "intent_head_path", project_root / "intent" / "large-intent.pt")
    )
    sv_ckpt = _optional_path(
        _cfg_get(
            cfg,
            "sv_ckpt",
            project_root / "model" / "wav2vec_large.ckpt",
        )
    )
    ...
    output_path = Path(
        _cfg_get(
            cfg,
            "output_path",
            project_root / "results" / "main" / "large_btuap_eval.json",
        )
    )

    print(
        "[*] Large BTUAP evaluation config: "
        f"large_backbone={large_backbone}, uap_path={uap_path}, "
    ...
    if run_sv and sv_ckpt is None:
        print(
            "[!] large_eval.sv_ckpt is not set. "
            "SV will use the local large pretrained backbone without a fine-tuned SV checkpoint."
        )
    ...
    sr_results = None
    if run_sv:
        sv_results = evaluate_large_sv(
            cfg=cfg,
            attack_cfg=attack_cfg,
            evaluator=evaluator,
            large_backbone=large_backbone,
            sv_ckpt=sv_ckpt,
            uap_delta=uap_delta,
            device=device,
    ...
    print(
        ...
        f"Conditional Flip Rate={combined['conditional_flip_rate']}, "
    ...
    with output_path.open("w", encoding="utf-8") as handle:
        json.dump(
            {
                "config": {
                    "large_backbone": str(large_backbone),
    ...
    print(f"[*] Saved large BTUAP evaluation: {output_path}")
